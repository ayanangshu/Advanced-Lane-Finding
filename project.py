...
##The following function transforms the image from a bird's eye view perspective. This will make it easier to fit 
##polynomials to the lane lines and measure the curvature


# In[5]:

# Perform perspective transform
def warped_image(img, display=True, read = True):
    if read:
    ...
    #The following part is manual. There is an opportunity to improve this
...
# ### I converted the warped image to different color spaces and create binary thresholded images which highlight only the lane lines and ignore everything else

# In[13]:

#I created a combined binary thresholded image based on S Channel from the HLS color space,The L Channel from 
#the LUV color space and The B channel from the Lab color space to create one combination of binary thresholded 
#image which highlights almost all of the white and yellow lane lines


# In[11]:

# Create binary thresholded images to isolate lane line pixels
def convert_bin_threshold(image, show=True):
    img, M = warped_image(image, display = False)
    ...
    s_thresh_min = 180 #Tried with 170 as well but 180 gave better results
    s_thresh_max = 255
...
for image in glob.glob('test_images/test*.jpg'):
    convert_bin_threshold(image)


# ## Now I  fit a polynomial to the lane lines by calculating the vehicle position and radius of curvature
...
def fit_polynomial(image):
    ...
    f, (ax1, ax2) = plt.subplots(1,2, figsize=(9, 6))
    f.tight_layout()
    ax1.imshow(cv2.cvtColor((warped_image(image, display=False)[0]), cv2.COLOR_BGR2RGB))
    ax1.set_xlim(0, 1280)
    ax1.set_ylim(0, 720)
...
#I establish a class Line() for the lines to store attributes about the lane lines from one 
#frame to the next. Inside the class there are several functions which will be used to detect the pixels 
#belonging to each lane line
...
white_clip = clip1.fl_image(get_video) 
...
challenge_clip = clip1.fl_image(get_video) 
...
clip1 = VideoFileClip("project_video_out_harder_challenge.mp4")
challenge_clip = clip1.fl_image(get_video) 
challenge_clip.write_videofile(challenge_output, audio=False)
